...
module axi4lite2axist #(
    ...
    // ---- 提前声明（修复 Verilog 前向引用：下列信号在下方 FIFO 例化的 assign 中被引用，
    //      原代码将其声明置于 RX/TX 状态机段，导致编译报 'Undefined variable'）----
    reg        tx_pop_wrq, tx_pop_rdq;   // TX 状态机弹出 WRQ/RDQ 的控制脉冲
    reg        tx_pop_hold;               // 弹出保持（覆盖 FWFT empty 滞后 1 拍窗口，防重发）
    reg        rx_push_b;                // RX 状态机推入 BRSP FIFO 的有效脉冲
    reg [1:0]  rx_push_bresp;            // 写响应 BRESP
    reg        rx_push_r;                // RX 状态机推入 RRSP FIFO 的有效脉冲
    reg [31:0] rx_push_rdata;            // 读响应 RDATA
    reg [1:0]  rx_push_rresp;            // 读响应 RRESP
    ...
    // aw_rd_en 由 make_wrq 驱动（make_wrq 定义见 WRQ 组装逻辑段，已置于 AW/W/WRQ 三 FIFO 之后）
    ...
    // w_rd_en 由 make_wrq 驱动（定义见 WRQ 组装逻辑段）
    ...
    // 修正 W 数据滞后（读-写碰撞）缺陷：
    //   原设计在配对弹出当拍直接从 AW/W FIFO 的 FWFT dout 组合 wrq_din。但当测试在
    //   配对同一拍向 W FIFO 写入新数据（awvalid/wvalid 同拍驱动）时，W FIFO 的 dout
    //   在该拍仍反映被弹出的旧字头，新数据要到下一拍才更新，导致 wrq_din 采到上一笔
    //   的写数据（WDATA 整体滞后一笔）。两拍 pair_pop 能侥幸自愈，是因为第二拍 wrq_din
    //   采到了已更新的新数据，但同时会带来重复配对（见 pair_armed）。
    //   此处改为：在 AW/W 各自被写入（aw_wr_en / w_wr_en 有效）的当拍，将测试侧的原始
    //   地址/数据/选通锁存到保持寄存器，wrq_din 直接由保持寄存器组合。数据在写入拍即
    //   已稳定，彻底摆脱 FWFT dout 的滞后与读-写碰撞，每笔只配对一次且数据正确。
    ...
    // wrq_wr_en 由 make_wrq 驱动（定义见 WRQ 组装逻辑段）
    ...
    //   定义置于 AW/W/WRQ 三组 FIFO 之后，确保 aw_empty/w_empty/wrq_full 均已声明
    //=====================================================================
    // =========================================================================
    // 修正 AW/W 配对缺陷（关键修复，定位见 sim/debug_log10.log 的 [WP] 追踪）：
    //   原设计用组合信号 make_wrq = !aw_empty && !w_empty && !wrq_full && !wrq_pair_lock
    //   同时驱动 aw_rd_en / w_rd_en / wrq_wr_en。而 make_wrq 又组合依赖于
    //   aw_empty/w_empty（FIFO 的寄存器输出），在单一仿真 delta 内形成
    //   "rd_en -> 弹出 -> empty 变化 -> make_wrq 变化 -> rd_en 变化" 的组合回环冒险。
    //   W FIFO 的 rd_en 在该 delta 内被"断言又撤销"，采样边沿上出现毛刺，导致 W FIFO
    //   偶发不弹出（AW FIFO 碰巧弹出），写数据相对地址整体滞后一笔
    //   （实测：每个写事务的 WDATA 都是上一笔的数据）。
    //   [WP] 证据：T=105ns 弹出后 aw_e 在 T=115ns 已为 1，而 w_e 仍为 0、w_dout 仍停留
    //   0x12345678 —— 即 W FIFO 未被弹出。
    //   修复：用寄存器 pair_pop 锁存弹出脉冲，使 rd_en 成为干净的寄存器输出（稳定保持
    //   一整个时钟），彻底消除组合冒险；配对条件仍依赖 FIFO empty/full，但 rd_en 不再
    //   参与组合回环。FWFT 模式下 dout 在 rd_en 当拍即反映被弹出的字头，故 wrq_din 仍
    //   组合取 dout，配对正确。
    // =========================================================================
    reg  pair_pop;
    reg  pair_det;
    reg  pair_armed;   // 单次配对锁：make_pair 依赖 AW/W FIFO 的 empty 信号，而 XPM
                       // FWFT FIFO 的 empty 在 rd_en 弹出后滞后约 1 拍更新，导致 make_pair
                       // 在弹出后仍持续多拍为高。若 pair_pop 直接打拍 make_pair，同一对
                       // AW/W 会被弹出并写入 WRQ 两次（重复配对），进而 TX 重复发包、
                       // native 重复执行、BRSP FIFO 累积多余响应、B 通道 bvalid 卡死。
                       // 用 pair_armed 在 make_pair 首个高电平触发一次后立即封锁，直到
                       // make_pair 回到低电平（FIFO 已排空）才解除，确保每对只配对一次。
    // 修正 W FIFO 读-写碰撞：make_pair 在 AW/W 双双非空当拍即有效，而该拍往往与测试向
    // W FIFO 写入新数据同拍。若当拍即弹出，XPM FWFT FIFO 在 rd_en 与 wr_en 同拍时弹出
    // 不可靠（读被写淹没），导致 W 字头滞留、上笔写数据残留，进而配对错乱（T2 分离写把
    // T1 的 W 与新 AW 配成一笔）。因此把真正弹出延后 1 拍：make_pair 有效当拍只置
    // pair_det，下一拍才 pair_pop，确保 W 写入已落定、dout 已更新，弹出干净且无碰撞。
    ...
    always @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            if (!make_pair) pair_armed <= 1'b0;
        end
    end
    assign aw_rd_en     = pair_pop;  // AW+W 配对时弹出（延后 1 拍，干净单拍脉冲，无组合冒险/无碰撞）
    assign w_rd_en      = pair_pop;
    assign wrq_wr_en    = pair_pop;
    // 兼容仿真追踪：make_wrq 现等价 pair_pop（仅供 tb 的 DEBUG 块引用）
    wire make_wrq = pair_pop;
    ...
    // 修正（T6 BRESP=OKAY 缺陷）：仅"非展示态(!bvalid)且 FIFO 非空"才读，
    // 避免 bvalid&&bready 同拍既清零又重读导致 bvalid 残留旧响应(stale)。
    assign brsp_rd_en = !s_axi_bvalid && !brsp_empty;
    ...
    // 修正（同 B 通道）：仅"非展示态(!rvalid)且 FIFO 非空"才读，消除 stale rvalid
    assign rrsp_rd_en = !s_axi_rvalid && !rrsp_empty;
    ...
    always @(*) begin
        ...
        case (tx_state)
            TX_IDLE: begin
                // 修正重复发包缺陷：XPM FIFO(FWFT) 的 empty 在 rd_en 弹出后
                // 滞后 1 拍更新。重发发生在两个连续周期：
                //   (1) 弹出当拍（tx_pop_wrq/rdq=1）：!tx_pop_wrq/!tx_pop_rdq 屏蔽；
                //   (2) 弹出后的 empty-lag 拍：此时弹出脉冲已归零，但 empty 尚未
                //       更新（仍为 0），必须用 tx_pop_hold（它在弹出脉冲之后下一拍
                //       才为 1）屏蔽。
                // 两者缺一不可：单用 !tx_pop_wrq 会在 empty-lag 拍漏判而重发；
                // 单用 !tx_pop_hold 会在弹出当拍漏判（hold 滞后一拍）而重发。
                if ((!wrq_empty || !rdq_empty) && !tx_pop_wrq && !tx_pop_rdq && !tx_pop_hold)
    ...
    always @(*) begin
        ...
        if (tx_state == TX_SEND) begin
            ...
            if (tx_idx == 3'd0) begin
                // 包头
                // 修正帧格式缺陷：tx_total 为 3 位、3'd2 亦为 3 位，
                // 若直接相减则第三字段仅 3 位宽，导致整字未按 8 位字节对齐，
                // 魔数 0xAA 被推离 [31:24]，native 侧 RX 永远收不到帧头。
                // 改为使用 8 位常量 8'd2，使长度字段恒为 8 位、整字 32 位对齐。
                m_axis_cmd_tdata = {FRAME_MAGIC_HEAD, tx_type, tx_total - 8'd2, 8'h00};
    ...
    always @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            tx_pop_wrq <= 1'b0;
            tx_pop_rdq <= 1'b0;
            // tx_pop_hold 在弹出脉冲之后的下一拍仍为 1，覆盖 FWFT empty 滞后窗口
            tx_pop_hold <= tx_pop_wrq || tx_pop_rdq;
    ...
    // 注：rx_push_b / rx_push_bresp / rx_push_r / rx_push_rdata / rx_push_rresp
    //     已在文件前部提前声明（修复前向引用），此处不再重复声明。
