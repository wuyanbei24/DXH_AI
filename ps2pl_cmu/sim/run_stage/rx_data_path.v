...
always @(*) begin
    ...
    case(curr_state)
        ...
        RX_WRITE: begin
            // [P-21] 仅当 pl_rx_valid 有效时才写 BRAM。原设计在 RX_WRITE 态无条件
            // 拉高 bram_wr_en，导致 pl_rx_valid 撤销的那一拍（RX_WRITE→RX_FINISH
            // 的转换拍）会向 addr=data_cnt 写入一拍“幻影”数据（stale/garbage），
            // 既多写一个无效字，又可能在 len 边界处污染后续读。这里把写使能与数据
            // 计数都收拢到 valid 有效的条件下。
            if(pl_rx_valid) begin
